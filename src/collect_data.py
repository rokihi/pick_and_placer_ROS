...
class CollectData(object):
    ...
    def workflow(self):
        ...
        target_pose.orientation = self.home_pose.orientation
    ...
    def image_saving(self, msg):
        print("Received an image! ---", self.c)
        try:
            # Convert your ROS Image message to OpenCV2
            cv2_img = bridge.imgmsg_to_cv2(msg, "bgr8")
        except CvBridgeError, e:
            ...
            # Save your OpenCV2 image as a jpeg
            cv2.imwrite('camera_image{0:04d}.jpeg'.format(self.c), cv2_img)
        self.subonce.unregister()
        # sleep(1.0)
        # rospy.sleep(1.0)

    def capture(self):
        self.subonce = None
        self.subonce = rospy.Subscriber(
            self.image_topic, Image, self.image_saving)
    ...
    def main(self):
        ...
        self.rarm.set_workspace(self.workspace)
        ...
        home_joints = [
            ...
            -0.7000814234168269]
        # self.rarm.set_pose_target(self.home_pose)
        self.rarm.set_joint_value_target(home_joints)
